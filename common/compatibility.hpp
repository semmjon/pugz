...
template<typename T, size_t N>
inline constexpr std::reverse_iterator<T*> rbegin(T (&arr)[N])
{
    return { arr + N };
}
template<typename T, size_t N>
inline constexpr std::reverse_iterator<T*> rend(T (&arr)[N])
{
    return { arr };
}
template<typename T, size_t N>
...
template<bool I, class T, class E>
using conditional_t = typename std::conditional_t<I, T, E>::type;
