...
#define ASSERT(expr)                                                                                                   \
    if (!(expr)) assertion_failed(__FILE__, __LINE__);
...
static void
test_multipart(const u8* buffer, unsigned size, const char* name, cksum_fn_t cksum, u32 v, u32 expected)
{
...
static void
test_checksums(const void* buffer,
               unsigned    size,
               const char* name,
               cksum_fn_t  cksum1,
               cksum_fn_t  cksum2,
               u32         initial_value)
{
    ...
    if (v1 != v2) {
        ...
        fprintf(stderr,
                "initial_value=0x%08" PRIx32 ", buffer=%p, "
                "contents ",
                initial_value,
                buffer);
...
tmain(int argc, tchar* argv[])
{
    ...
    for (uint32_t i = 0; i < 50000; i++) {
        ...
        test_checksums(&buffer[start], len, "Adler-32", libdeflate_adler32, zlib_adler32, select_initial_adler());

        test_checksums(&buffer[start], len, "CRC-32", libdeflate_crc32, zlib_crc32, select_initial_crc());
    }
