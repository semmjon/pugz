...
class span
{
  public:
    ...
    template<typename R>
    auto includes(const R& r) -> decltype(includes(r.begin, r.end)) const restrict
    {
...
class unique_span : private D
{
  public:
    ...
    template<typename R,
             typename _D = D,
             typename = typename std::enable_if<std::is_nothrow_constructible<D, _D&&>::value &&
    ...
    template<typename U>
    span<const U> reinterpret() const
    {
        assert((size() * sizeof(T)) % sizeof(U) == 0);
        assert(reinterpret_cast<uintptr_t>(_begin) % alignof(U) == 0);
...
template<typename T>
using malloc_span = unique_span<T, free_deleter<T[]>>;
...
alloc_huge(size_t n)
{
    ...
    T* ptr;
    if (posix_memalign(reinterpret_cast<void**>(&ptr), details::huge_page_size, bytes) != 0)
        throw std::bad_alloc();
...
template<typename T>
using mmap_span = unique_span<T, mmap_deleter<T[]>>;
